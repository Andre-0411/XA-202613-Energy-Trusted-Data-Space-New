...
const DataAssetsPage: React.FC = () => {
  ...
  const stats = useMemo(() => ({
    ...
    sensitive: items.filter(i => i.sensitivity_level === 'confidential' || i.sensitivity_level === 'secret').length,
    byCategory: {
      electricity: items.filter(i => i.category === 'electricity').length,
      gas: items.filter(i => i.category === 'gas').length,
      renewable: items.filter(i => i.category === 'renewable').length,
      market: items.filter(i => i.category === 'market').length,
      device: items.filter(i => i.category === 'device').length,
      geographic: items.filter(i => i.category === 'geographic').length,
    },
    bySensitivity: {
      public: items.filter(i => i.sensitivity_level === 'public').length,
      internal: items.filter(i => i.sensitivity_level === 'internal').length,
      confidential: items.filter(i => i.sensitivity_level === 'confidential').length,
      secret: items.filter(i => i.sensitivity_level === 'secret').length,
    },
  }), [items, total]);

...
const DashboardPage: React.FC = () => {
  ...
  const statCards = useMemo(() => {
    const widgetDataMap: Record<string, { value: number; unit: string; icon: React.ReactNode; gradient: string; trend: number }> = {
      ...
      pending_requests: { value: kpi?.pending_requests ?? 0, unit: '个', icon: <ErrorCircleIcon size="20px" />, gradient: 'orange', trend: -5.3 },
      data_usage: { value: kpi?.data_usage_gb ?? 0, unit: 'GB', icon: <ServerIcon size="20px" />, gradient: 'purple', trend: 18.7 },
      my_tasks: { value: kpi?.my_tasks ?? 0, unit: '个', icon: <ChartBarIcon size="20px" />, gradient: 'blue', trend: 3.2 },
      available_assets: { value: kpi?.available_assets ?? 0, unit: '项', icon: <ServerIcon size="20px" />, gradient: 'green', trend: 8.5 },
      my_usage: { value: kpi?.my_usage_gb ?? 0, unit: 'GB', icon: <CloudIcon size="20px" />, gradient: 'purple', trend: 12.1 },
      notifications: { value: kpi?.unread_notifications ?? 0, unit: '条', icon: <NotificationIcon size="20px" />, gradient: 'orange', trend: -2.3 },
      compliance_score: { value: kpi?.compliance_score ?? 88, unit: '分', icon: <ShieldErrorFilledIcon size="20px" />, gradient: 'green', trend: 1.8 },
      security_incidents: { value: kpi?.security_incidents ?? 0, unit: '个', icon: <ShieldErrorFilledIcon size="20px" />, gradient: 'red', trend: -15.2 },
      audit_logs: { value: kpi?.audit_log_count ?? 0, unit: '条', icon: <HistoryIcon size="20px" />, gradient: 'blue', trend: 22.5 },

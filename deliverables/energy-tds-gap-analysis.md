...
|------|---------|--------|---------|----------|------|--------|
| 统一门户 | 17 | 7 | 5 | 3 | 2 | 41% |
| 数据资源中心 | 20 | 9 | 4 | 4 | 3 | 45% |
| 可信计算中心 | 27 | 6 | 5 | 12 | 4 | 22% |
| 区块链存证中心 | 22 | 5 | 4 | 8 | 5 | 23% |
| 运营管理中心 | 23 | 6 | 5 | 7 | 5 | 26% |
| 安全管控中心 | 28 | 5 | 6 | 10 | 7 | 18% |
| 基础设施层 | 12 | 3 | 2 | 4 | 3 | 25% |
| **总计** | **149** | **41** | **31** | **48** | **29** | **28%** |

### 1.2 关键发现

1. **"有壳无核"问题严重**: 约 48 项功能（32%）为存根/模拟实现，前端页面和 API 路由已就位，但后端逻辑回退到内存字典或硬编码返回值
2. **内存存储泛滥**: 至少 13 个 service 文件使用纯内存字典（`_store`/`_sessions`）替代数据库持久化
3. **隐私计算全链路降级**: FATE、MPC、TEE、HE 四条技术路线均回退为本地模拟，未连接任何真实计算引擎
4. **国密算法正确性存疑**: SM2 密钥生成返回空公钥（`gmssl_adapter.py:101`），SM9 使用 SM3 哈希伪造签名
5. **响应式设计覆盖率极低**: 仅 `MainLayout.tsx` 和 `LandingPage.tsx` 两个文件使用了 `useMediaQuery`
...
| 2.1.2.1 | 传统账号密码登录 | ✅ 已实现 | P0 | `auth_service.py` + `auth.py` 路由已实现 | 无 |
| 2.1.2.1 | 国密证书登录 | ⚠️ 部分实现 | P1 | `security_gmssl.py` 路由存在，但底层依赖 GmSSL 库 | 需确保 GmSSL 正确安装并测试 |
| 2.1.2.1 | 强制双因素认证（MFA） | ⚠️ 存根 | P0 | `mfa_service.py` 实现了 TOTP 协议逻辑（HMAC-SHA1 + 时间窗口验证），但 **所有数据存储在内存字典**（`_mfa_store`、`_backup_codes_store`、`_mfa_sessions`），重启即丢失 | 1) 将 `_mfa_store` 迁移到 PostgreSQL MfaCredential 模型 2) 将 `_backup_codes_store` 迁移到数据库 3) 将会话存储接入 Redis |
...
| 2.1.3.1 | 响应式设计（1920x1080 + 1366x768） | ❌ 缺失 | P1 | 仅 `MainLayout.tsx:25` 和 `LandingPage.tsx` 使用了 `useMediaQuery`，其余 45+ 页面未做响应式适配 | 1) 在 MUI 主题中配置 breakpoints 2) 所有页面组件使用 Grid/Stack 响应式布局 3) 增加 `sx={{ display: { xs: 'none', md: 'block' } }}` 模式 |
...
| 2.1.4.1 | SSO单点登录 | ⚠️ 存根 | P0 | `sso_service.py` 实现了 OAuth2.0/OIDC/SAML2.0 的协议框架，但 **Token 交换使用硬编码模拟**（`exchange_token` 返回 mock 用户信息），所有 Provider 配置存储在内存字典 `_sso_providers` | 1) 接入真实的 OAuth2 Provider（如 Keycloak） 2) 将 Provider 配置迁移到数据库 3) 实现真实的 Token 交换逻辑 |
| 2.1.4.2 | 登录失败5次锁定 | ❌ 缺失 | P0 | 未发现任何登录失败计数或账户锁定逻辑 | 在 `auth_service.py` 中增加 Redis 计数器：`login_fail:{user_id}`，5 次失败后锁定 30 分钟 |
| 2.1.4.3 | CSRF防护 / XSS过滤 | ❌ 缺失 | P0 | `deploy/nginx/nginx.conf` 中有安全头配置，但后端未实现 CSRF token 中间件 | 1) 使用 `starlette-csrf` 中间件 2) 在 Nginx 配置中添加 `X-Content-Type-Options`、`X-Frame-Options` 等头 |
| 2.1.4.4 | 审计日志 | ✅ 已实现 | P1 | `audit_log.py` + `AuditLogPage.tsx` | 无 |
| 2.1.5 | WebSocket实时推送 | ⚠️ 部分实现 | P1 | `ws_manager.py` 实现了 4 端点连接管理（notifications/compute/monitor/threats），支持广播和定向发送，但 **`send_to_user` 方法降级为广播**（`ws_manager.py:81`），无用户-连接映射 | 1) 在 `connect` 时存储 `user_id → websocket` 映射 2) 实现真正的定向消息投递 |
...
| 2.2.2.3 | 实时采集（核心 ≤1秒） | ⚠️ 部分实现 | P0 | `mqtt_simulator.py` 实现了 5 个模拟发电企业的数据采集，但 **数据存储在内存环形缓冲区**（`mqtt_data_store.py` 使用 `deque(maxlen=3600)`），重启即丢失 | 1) 将采集数据同时写入 PostgreSQL（持久化）和 Redis（实时缓存） 2) 使用 TimescaleDB 扩展优化时序数据存储 |
...
| 需求项 | 需求描述 | 当前状态 | 优先级 | 具体问题 | 修复建议 |
|--------|---------|---------|--------|---------|---------|
| 2.3.2.1 | 联邦学习（FATE） | ⚠️ 存根 | P0 | `fate_integration.py` 实现了 FATE Flow REST API 客户端（httpx 异步调用），但 **连接失败时自动降级为本地 sklearn 模拟**（`_local_simulation_submit`），且任务存储在内存字典 `_fate_jobs` | 1) 部署 FATE Flow 服务（Docker） 2) 将任务状态持久化到 PostgreSQL 3) 移除或标记模拟模式 |
...
**智能合约清单**（contracts/ 目录，共 18 个文件）：

| 合约 | 功能 | 接口 |
|------|------|------|
| IdentityRegistry.sol | DID 身份注册 | ✅ IIdentityRegistry.sol |
| DataAssetNFT.sol | 数据资产 NFT | ✅ IDataAssetNFT.sol |
| AccessControl.sol | 访问控制 | ✅ IAccessControl.sol |
| UsageLogger.sol | 使用日志 | ✅ IUsageLogger.sol |
| AutoSettlement.sol | 自动结算 | ✅ IAutoSettlement.sol |
| ComplianceAudit.sol | 合规审计 | ✅ IComplianceAudit.sol |
| DataAssetRights.sol | 数据资产确权 | ❌ |
| DataAuthorization.sol | 数据授权 | ❌ |
| DataTraceability.sol | 数据溯源 | ❌ |
| DataTrading.sol | 数据交易 | ❌ |
| IncentiveDistribution.sol | 激励分配 | ❌ |
...
| 2.6.6 | 国密算法集成（SM2/SM3/SM4） | ⚠️ 存根 | P0 | `gmssl_adapter.py` 实现了 SM2 签名/验签/加密/解密、SM3 哈希、SM4 加密/解密的完整接口，但 **存在严重问题**：1) SM2 密钥生成返回空公钥（`sm2_generate_keypair` 第 101 行 `return private_key, ""`） 2) SM9 使用 SM3 哈希伪造签名（第 174 行） 3) ZUC 使用 SM4 降级（第 197 行） 4) GmSSL 库不可用时 SM2 直接抛出异常而非降级 | 1) 修复 SM2 密钥对生成（使用 `gmssl` 的 `sm2_key_pair_generate`） 2) 移除 SM9/ZUC 的伪实现 3) 增加 GmSSL 安装验证 |
...
以下 13 个服务文件使用纯内存字典替代数据库持久化，**重启即丢失所有数据**：

| 服务文件 | 内存存储变量 | 影响范围 | 优先级 |
|---------|------------|---------|--------|
| `mfa_service.py` | `_mfa_store`, `_backup_codes_store`, `_mfa_sessions` | MFA 配置、备份码、会话 | **P0** |
| `sso_service.py` | `_sso_providers`, `_sso_sessions`, `_pending_auth` | SSO 提供者配置、会话 | **P0** |
...
| `mqtt_data_store.py` | `_data`, `_devices`, `_alarms` | MQTT 采集数据、设备注册、告警 | **P0** |
...
| 问题 | 文件位置 | 严重性 | 描述 | 修复建议 |
|------|---------|--------|------|---------|
| SM2 密钥生成返回空公钥 | `gmssl_adapter.py:101` | **严重** | `sm2_generate_keypair()` 返回 `(private_key, "")`，空公钥将导致所有依赖公钥的操作失败 | 使用 GmSSL 的 `sm2_key_pair_generate` 正确生成密钥对 |
...
4. **表格**: 使用 `DataTable` 组件的响应式模式（横向滚动或列折叠）
5. **图表**: ECharts 使用 `resize` 事件自动适配容器大小
...
| 5 | 修复 SM2 密钥对生成 | 安全 | 0.5 天 | GmSSL 库 |
| 6 | MFA 服务迁移到 PostgreSQL | 门户 | 1 天 | #1 |
| 7 | SSO 服务迁移到 PostgreSQL | 门户 | 1 天 | #1 |
| 8 | MQTT 数据存储迁移到 PostgreSQL + Redis | 数据资源 | 1 天 | #1, #2 |
| 9 | 登录失败锁定机制 | 门户 | 0.5 天 | #2 |
| 10 | CSRF/XSS 安全中间件 | 门户 | 0.5 天 | 无 |
| 11 | FATE 联邦学习任务持久化 | 可信计算 | 1 天 | #1 |
| 12 | DID 身份认证 + SM2 签名完整流程 | 安全 | 1 天 | #5 |
| 13 | 数据资产上链存证 | 区块链 | 1 天 | #3, #4 |
| 14 | 操作存证全链路覆盖 | 区块链 | 1 天 | #13 |
| 15 | ABAC 策略引擎完善 | 安全 | 1 天 | 无 |
| 16 | 基础权限管控验证 | 门户 | 0.5 天 | 无 |
| 17 | WebSocket 用户定向消息 | 门户 | 0.5 天 | #2 |
| 18 | 统一门户多角色登录验证 | 门户 | 0.5 天 | 无 |
...
| 24 | 响应式设计全面适配 | 门户 | 3 天 |
| 25 | 监管大屏实时数据接入 | 门户 | 1 天 |
| 26 | Prometheus 指标采集真实接入 | 运营 | 1 天 |
| 27 | 告警推送通道集成 | 运营 | 1 天 |
| 28 | 月度账单自动生成 | 运营 | 1 天 |
| 29 | VC 可验证凭证真实签发 | 安全 | 2 天 |
| 30 | 密钥轮换机制 | 安全 | 1 天 |
| 31 | 三层密钥体系 | 安全 | 1 天 |
| 32 | Shamir 秘密共享 | 安全 | 1 天 |
| 33 | HSM 模拟完善 | 安全 | 1 天 |
| 34 | 实时威胁检测规则引擎 | 安全 | 2 天 |
| 35 | 安全态势大屏真实数据 | 安全 | 1 天 |
| 36 | 数据沙箱 seccomp 策略 | 可信计算 | 1 天 |
| 37 | DAG 可视化增强 | 可信计算 | 1 天 |
| 38 | 自动计费结算智能合约 | 区块链 | 1 天 |
| 39 | NFT 铸造完整流程 | 区块链 | 1 天 |
| 40 | 确权证书生成 | 区块链 | 1 天 |
| 41 | 组织架构完善 | 运营 | 1 天 |
| 42 | 用户全生命周期完善 | 运营 | 1 天 |
| 43 | 计费模式完善 | 运营 | 1 天 |
...
1. MFA/SSO 服务迁移到 PostgreSQL
2. MQTT 数据存储迁移到 PostgreSQL + Redis
3. FATE/MPC/TEE/HE 任务持久化
4. 计费/SLA/监控数据持久化
...
| 内存存储迁移工作量大 | 中 | 13 个服务需要修改 | 按优先级分批迁移，P0 服务优先 |
...
1. **32% 的功能为存根/模拟实现**，缺乏真实的后端逻辑
2. **18 个服务使用内存字典** 替代数据库，重启即丢失数据
